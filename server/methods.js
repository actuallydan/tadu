...
Meteor.methods({
  ...
  addTask(task, user) {
    ...
    /* Find tag in array  */
    let index = userTags.tags.findIndex((tag) => {
      return tag.type === task.tagType;
    });
    /* Increment */
  ...
  addTag(tag, thisUser) {
    ...
    if (
      user.tags.findIndex((thisTag) => {
        return thisTag.type.toLowerCase() === tag.toLowerCase();
      }) !== -1
    ) {
      return "exists";
    }
  ...
  addDefaultTags(thisUser) {
    ...
    tags.map((tag) => {
      myTags.tags.push({ type: tag, uses: 0 });
    });
  ...
  addDefaultSchedule(thisUser) {
    ...
    /* From the user's profile, get their bedtime and use that as an offset for their actual bioCurve. Then pass their bioCurve into the Thresholds Object parameters */
    let myBioCurve = offsetBioCurve(
      parseInt(thisUser.profile.bedHour.substring(0, 2))
    );
    let schedule = {
      userId: thisUser._id,
      schedule: Schedule(),
  ...
  modifySchedule(coords, thisUser) {
    ...
    mySchedule.schedule[day][hour] =
      mySchedule.schedule[day][hour] === null ? { type: "work" } : null;
  ...
  scheduleBestTime(data, thisUser) {
    ...
    const offset = parseInt(
      moment(data.today, "YYYY-MM-DDTHH:mm:ss").format("e")
    );
    const thisHour = moment(data.today, "YYYY-MM-DDTHH:mm:ss").format("HH:00");
    const todayFormatted = moment(data.today, "YYYY-MM-DDTHH:mm:ss").format(
      "YYYY-MM-DD"
    );
    ...
    if (!mySched.thresholds["Sun"]["00:00"].hasOwnProperty(data.tag)) {
      daysOfWeek.map((day) => {
        hours.map((hour) => {
          mySched.thresholds[day][hour][data.tag] =
            bioCurve[hours.indexOf(hour)];
        });
      });
    ...
    let tasks = Tasks.find({
      ...
      dateStart: {
        gte: todayFormatted,
        lte: moment(todayFormatted, "YYYY-MM-DD")
          .add(7, "days")
          .format("YYYY-MM-DD"),
      },
    }).fetch();
    ...
    while (target > 0.1) {
      ...
      tempPossibilites = tempPossibilites.filter((coord) => {
        let daysFromToday =
          coord.day - offsetBioCurve < 0
            ? 7 - Math.abs(coord.day - offset)
            : coord.day - offset;
        // console.log("At ", coord, " there is a " + mySched.thresholds[daysOfWeek[coord.day]][coord.time][data.tag] + " chance")
        return (
          mySched.thresholds[daysOfWeek[coord.day]][coord.time][data.tag] -
            daysFromToday * 0.01 >=
          target
        );
      });
      ...
      tempPossibilites = tempPossibilites.filter((coord) => {
        ...
        return (
          tasks.findIndex((task) => {
            return (
              task.dateStart === bestDate.substring(0, 10) &&
              task.timeStart === coord.time
            );
          }) === -1
        );
      });
...
const Thresholds = (myBioCurve) => {
  ...
  daysOfWeek.map((day) => {
    hours.map((hour) => {
      self[day][hour] = HourThresholdObj(hour, myBioCurve);
    });
  });
  return self;
};
...
const HourThresholdObj = (hour, myBioCurve) => {
  let self = {};
  tags.map((tag) => {
    self[tag] = myBioCurve[hours.indexOf(hour)];
  });
  return self;
};

	...
	*/
	...
	selectDate(date){
		if(document.getElementById("new-task-date") !== null){
			...
			document.getElementById("new-task-end-date").value = moment(date, "YYYY-MM-DD").add(1, 'hour').format("YYYY-MM-DD");
		} 
		this.setState({
			selectedDate: date
		});
	...
	*/
	...
	render(){
		...
		let filteredTasks = Tasks.find().fetch().filter(
			...
			<div id="close-task-detail" onClick={this.hideDetail.bind(this)} className="mdi mdi-close"> </div>

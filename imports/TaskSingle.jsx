  ...
  return (
    <li className="task-single animated bounceInUp" onClick={showDetail}>
      <div
        ...
        style={{
          visibility:
            props.task.userId === Meteor.userId() ? "visible" : "hidden",
        }}
      >
        <input
          id={"task_" + props.task._id}
          type="checkbox"
          readOnly={true}
          checked={props.task.completed}
          onClick={() => {
            Meteor.apply("toggleTask", [props.task, Meteor.user()]);
          }}
        />
      ...
      <div>
        <span
          className={
            props.task.completed ? "task-title completed-task" : "task-title"
          }
        >
          {props.task.text}
        </span>
        <p className="task-info-date">
          {moment(props.task.dateStart, "YYYY-MM-DD").format("M/DD/YYYY")}
        </p>
        <p className="task-info-time">
          {moment(props.task.timeStart, "HH:mm").format("h:mm a")}
        </p>
      </div>
      {props.task.userId === Meteor.userId() ? (
        <i
          className="btn-cancel task-secondary mdi mdi-close-box-outline text-grey-light"
          onClick={deleteTask}
        ></i>
      ) : (
